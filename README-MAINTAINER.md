...
Determine the upstream version (like `2.0.1`) and eventually update the
`package.json` file; the format is `2.0.1-pre`.
...
and fix them; assign them to a milestone (like `2.0.1`).
...
- add a new entry like _\* v2.0.1_
...
- `npm dist-tag add @micro-os-plus/devices-qemu-riscv@2.0.1 latest`
- `npm dist-tag ls @micro-os-plus/devices-qemu-riscv`
